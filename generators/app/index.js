...
module.exports = class extends Generator {
    ...
    async prompting() {
        ...
        this.answers = await this.prompt([
            ...
            },
            {
                type: "confirm",
                name: "docker",
                message: "Would you like to deploy app on Docker?",
                default: false
            },
            {
                type: "confirm",
                name: "pwa",
                message: "Would you like to install PWA (manifest and service worker)?",
                default: false
            },
            {
                type: "confirm",
    ...
    writing() {
        ...
        if (this.answers.docker) {
            this.log(chalk.bold.blue("--------------------"));
            this.log(chalk.bold.blue("[Docker] coping files"));
            this.log(chalk.bold.blue("--------------------"));
            this.fs.copy(
                this.templatePath('_scripts/_run.sh'),
                this.destinationPath(this.answers.appname + '/scripts/run.sh')
            );
            this.log(chalk.bold.green("[Docker] Successfully copied run.sh"));
            this.fs.copy(
                this.templatePath('_scripts/_tests.sh'),
                this.destinationPath(this.answers.appname + '/scripts/tests.sh')
            );
            this.log(chalk.bold.green("[Docker] Successfully copied tests.sh"));
            this.fs.copy(
                this.templatePath('_default.conf'),
                this.destinationPath(this.answers.appname + '/default.conf')
            );
            this.log(chalk.bold.green("[Docker] Successfully copied default.conf for Nginx"));
            this.fs.copy(
                this.templatePath('_Dockerfile'),
                this.destinationPath(this.answers.appname + '/Dockerfile')
            );
            this.log(chalk.bold.green("[Docker] Successfully copied Dockerfile"));
        }

        // Copy PWA
        if (this.answers.pwa) {
            this.log(chalk.bold.blue("--------------------"));
            this.log(chalk.bold.blue("[PWA] Installing"));
            this.log(chalk.bold.blue("--------------------"));
            this.spawnCommandSync('npm', ['add', '@angular/pwa']);
            this.log(chalk.bold.green("[PWA] Successfully installed globally"));
        }
